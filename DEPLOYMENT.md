...
| Region | `us-central1` |
| Service name | `cart-checkout-service` |
...
```bash
docker build -t cart-checkout-service:local .
docker run --rm -p 8080:8080 cart-checkout-service:local
```

## Manual Cloud Run Deploy

```bash
IMAGE=us-central1-docker.pkg.dev/probestack-prod/probestack-prod-apps/cart-checkout-service:manual

docker build -t ${IMAGE} .
docker push ${IMAGE}

gcloud run deploy cart-checkout-service \
  --image=${IMAGE} \
  --platform=managed \
  --region=us-central1 \
